...
const ThumbnailImages = () => {
	...
	const projectImgs = useSelector((state) =>
		Object.values(state.image.images)
	);
	...
	const imageDivCreate = (num, array) => {
		...
		if (array[0] !== undefined) {
			for (let i = 1; i < num; i++) {
				url = array[i];
				newThumbnail = (
					<div key={i} className='h-16 w-16 m-2 cursor-pointer'>
						<a onClick={(e) => setMainImage(e.target.src)}>
	...
	return (
		<div className='flex border-b-2'>{displayDivs.map((ele) => ele)}</div>
	);
};
